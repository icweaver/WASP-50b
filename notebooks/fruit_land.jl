...
# ╔═╡ 31bd0b0c-1add-11ec-3af9-67f3ead1f846
using AlgebraOfGraphics, CairoMakie, DataFrames, DataFramesMeta

# ╔═╡ e44706a2-95ea-4c50-89c5-257b7e6dba93
set_aog_theme!()
...
md"""
Just going to pick out some random cities here for funsies:
"""

# ╔═╡ 8dada509-93f0-494c-ad6c-c864bf0b0a56
df_sub = @subset df :city .∈ Ref(["Kiwi", "Orange", "Watermelon"])
...
let
m = mapping(:size => "Size (km²)", :pop =>"Population (million)";
	color = :cost => "Cost (Schrute Bucks)",
	markersize = :happiness => (x -> 10.0*x), # Let's scale this up for visibility
)

marker_open, marker_closed = visual(marker='○'), visual(marker='●')

plt = m * (data(df)*marker_open + data(df_sub)*marker_closed)
